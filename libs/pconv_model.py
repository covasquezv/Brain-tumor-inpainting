...
from libs.VGG16_gray import VGG16


class PConvUnet(object):

    def __init__(self, img_rows=256, img_cols=256, vgg_weights="imagenet", inference_only=False, net_name='default', gpus=1, vgg_device=None):
        ...
        # Luminosity formula : (feature_red * 0.2989) + (feature_green * 0.5870) + (feature_blue * 0.1140)
        self.mean = [0.459] # [r, g, b] = [0.485, 0.456, 0.406]
        self.std = [0.226]  # [r, g, b] = [0.229, 0.224, 0.225]
    ...
    def build_pconv_unet(self, train_bn=True):
        ...
        outputs = Conv2D(1, 1, name='outputs_img', kernel_initializer=glorot_uniform(seed=0))(d_conv16)
        out_sigm = Activation('sigmoid')(outputs)

        # Setup the model inputs / outputs
        model = Model(inputs=[inputs_img, inputs_mask], outputs=out_sigm)
    ...
    def compile_pconv_unet(self, model, inputs_mask, lr=0.0002):
        print('Starting compiling PCONV ...')
        model.compile(
    ...
    def loss_total(self, mask):
        ...
        def loss(y_true, y_pred):
            ...
            y_comp = K.reshape(y_comp, (-1, 256, 256, 1))
    ...
    def predict(self, sample, **kwargs):
        """Run prediction using thi
        print(type(masked))s model"""
        return self.model.predict(sample, **kwargs)

    def evaluate(self, sample, label, **kwargs):
        """Run evaluation using this model"""
        return self.model.predict(sample, label, 10)
